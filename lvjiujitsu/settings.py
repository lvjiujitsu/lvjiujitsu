...
from system.core.environment import (
    EnvironmentContract,
    base_dir_path_setting,
    load_config,
    parse_db_url,
)
...
ENVIRONMENT_CONTRACT = EnvironmentContract(
    base_dir=BASE_DIR,
    shared_dir_variable="LVJIUJITSU_SHARED_ENV_DIR",
    default_shared_dir=r"W:\Meu Drive\Desenvolvimento\lvjiujitsu",
)

config = load_config(ENVIRONMENT_CONTRACT)
...
INSTALLED_APPS = [
    ...
    "system.core.apps.CoreConfig",
    "system.business_rule.apps.BusinessRuleConfig",
]
...
MIDDLEWARE = [
    ...
    "system.core.access.middleware.IdentityMiddleware",
]
...
CONTEXT_PROCESSORS = [
    ...
    "system.core.appearance.site_identity",
]
...
TEST_RUNNER = "system.core.test_runner.ProjectDiscoverRunner"
...
if DATABASE_URL:
    DATABASES = {"default": parse_db_url(config, DATABASE_URL)}
else:
    DATABASES = {
        "default": {
...
AUTH_USER_MODEL = "business_rule.User"
...
STATIC_ROOT = base_dir_path_setting(
    ENVIRONMENT_CONTRACT, config, "DJANGO_STATIC_ROOT", BASE_DIR / "staticfiles"
)
STATIC_ROOT.mkdir(parents=True, exist_ok=True)

MEDIA_URL = config("DJANGO_MEDIA_URL", default="/media/")
MEDIA_ROOT = base_dir_path_setting(
    ENVIRONMENT_CONTRACT, config, "DJANGO_MEDIA_ROOT", BASE_DIR / "media"
)
...
PASSWORD_RESET_TIMEOUT = PORTAL_PASSWORD_RESET_TOKEN_HOURS * 3600
...
# ----------------------------------------------------------------------------
# BN - business rule: academia, graduação, planos e cobrança por gateway
# externo. Acima desta linha está a base técnica, salvo as linhas marcadas
# uma a uma. Registro em obsidian/projetos/lvjiujitsu/.
# ----------------------------------------------------------------------------
...
ENV_TOLERATED_EMPTY = {
    **ENV_TOLERATED_EMPTY,
    ...
    ".env.hg": {"BREVO_API_KEY"},
    ".env.prod": {"BREVO_API_KEY"},
}
