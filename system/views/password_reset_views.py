...
from system.forms.password_reset_forms import (
...
class PasswordResetConfirmView(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if token == INTERNAL_RESET_URL_TOKEN:
            ...
            return HttpResponseRedirect(
                request.path.replace(token, INTERNAL_RESET_URL_TOKEN)
            )
    ...
    def form_valid(self, form):
        set_account_password(self.account, form.cleaned_data["new_password1"])
        self.request.session.pop(INTERNAL_RESET_SESSION_TOKEN, None)
        record_password_reset(self.request, self.kind, self.account)
        return super().form_valid(form)
