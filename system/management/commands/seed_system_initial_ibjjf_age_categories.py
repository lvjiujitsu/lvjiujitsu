...
class Command(BaseCommand):
    help = "Cria as categorias de idade IBJJF a partir de static/initial_data/ibjjf_age_categories.json."
    ...
    def handle(self, *args, **options):
        ...
        if not data:
            self.stdout.write(self.style.WARNING("Nenhuma categoria encontrada no arquivo ibjjf_age_categories.json."))
            return
    ...
    def _load_json(self) -> list:
        path = Path(settings.BASE_DIR) / "static" / "initial_data" / "ibjjf_age_categories.json"
        if not path.exists():
