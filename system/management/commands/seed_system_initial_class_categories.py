...
class Command(BaseCommand):
    help = "Cria as categorias de turma a partir de static/initial_data/class_categories.json."

    def handle(self, *args, **options):
        self.stdout.write(self.style.MIGRATE_HEADING("seed_system_initial_class_categories"))
        data = self._load_json()

        if not data:
            self.stdout.write(self.style.WARNING("Nenhuma categoria encontrada no arquivo class_categories.json."))
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\nCategorias de turma: {created_count} criada(s), {updated_count} atualizada(s)."
            )
        )

    def _load_json(self) -> list:
        path = Path(settings.BASE_DIR) / "static" / "initial_data" / "class_categories.json"
        if not path.exists():
    ...
    def _apply_updates(self, category: ClassCategory, entry: dict) -> None:
