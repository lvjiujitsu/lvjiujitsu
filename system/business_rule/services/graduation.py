...
def _rule_window(current, rule, reference_date):
    if rule.min_classes_window_months and rule.min_classes_window_months > 0:
        return add_months(reference_date, -rule.min_classes_window_months)
    return current.awarded_at


NO_GRADUATION_BLOCKER = "Sem registro de graduação."
NO_RULE_BLOCKER = "Sem regra de graduação cadastrada para esta faixa/grau."
...
def compute_graduation_progress(person, reference_date=None):
    ...
    if current is None:
        return _empty_progress(person, reference_date, NO_GRADUATION_BLOCKER)
    ...
    window_start = _rule_window(current, rule, reference_date)
    return _build_progress(
        ...
        approved_classes=count_approved_classes_in_window(
            person, window_start, reference_date
        ),
        months_in_current_grade=months_in_current_grade,
    )
...
def compute_graduation_progress_bulk(persons, reference_date=None):
    ...
    for pid, current in current_by_person.items():
        ...
        if rule is None:
            continue
        windows[pid] = (_rule_window(current, rule, reference_date), reference_date)
    ...
    for pid, person in person_by_id.items():
        ...
        if current is None:
            results[pid] = _empty_progress(person, reference_date, NO_GRADUATION_BLOCKER)
            continue
