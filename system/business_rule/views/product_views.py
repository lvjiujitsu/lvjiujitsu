...
from system.business_rule.forms.product_forms import (
    ProductCartForm,
    ProductCategoryForm,
    ProductForm,
    get_product_variant_formset,
)
from system.business_rule.models.product import Product, ProductCategory, ProductVariant
from system.business_rule.models.product_backorder import ProductBackorder
from system.business_rule.models.registration_order import PaymentStatus, RegistrationOrder
from system.business_rule.constants import MATERIAL_REQUEST_PERSON_TYPE_CODES
from system.business_rule.models import Person
from system.business_rule.selectors.product_backorders import (
...
from system.business_rule.services.product_backorders import (
    ProductBackorderError,
    cancel_backorder,
    confirm_backorder,
    create_backorder,
)
from system.business_rule.services.product_management import (
...
from system.business_rule.selectors.product_backorders import has_active_backorder_for_variant
...
class ProductListView(AdministrativeRequiredMixin, ListView):
    template_name = "business_rule/products/product_list.html"
...
class ProductCreateView(
    ...
    template_name = "business_rule/products/product_form.html"
...
class ProductDetailView(AdministrativeRequiredMixin, DetailView):
    template_name = "business_rule/products/product_detail.html"
...
class ProductDeleteView(AdministrativeRequiredMixin, DeleteView):
    model = Product
    template_name = "business_rule/products/product_confirm_delete.html"
    success_url = reverse_lazy("system:product-list")
    context_object_name = "product"
...
class ProductStoreView(PortalRoleRequiredMixin, ListView):
    ...
    template_name = "business_rule/products/product_store.html"
    context_object_name = "products"
    ...
    def get_context_data(self, **kwargs):
        ...
        context["purchase_person_default"] = portal_person(self.request)
        return context
    ...
    def _build_purchase_person_choices(self):
        actor = portal_person(self.request)
        if actor is None and is_technical_admin(self.request):
            return list(
...
class StudentBackorderListView(PortalRoleRequiredMixin, ListView):
    allowed_codes = MATERIAL_REQUEST_PERSON_TYPE_CODES
    template_name = "business_rule/products/student_backorder_list.html"
    context_object_name = "backorders"

    def get_queryset(self):
        recipients = get_material_request_recipient_queryset(portal_person(self.request))
...
class StudentOrderHistoryView(PortalRoleRequiredMixin, ListView):
    allowed_codes = MATERIAL_REQUEST_PERSON_TYPE_CODES
    template_name = "business_rule/products/student_order_history.html"
...
class AdminBackorderQueueView(AdministrativeRequiredMixin, ListView):
    template_name = "business_rule/billing/admin_backorder_queue.html"
    context_object_name = "backorders"
...
def _resolve_purchase_person_or_message(request):
    ...
    if actor is None and is_technical_admin(request):
        try:
...
class ProductCategoryListView(AdministrativeRequiredMixin, ListView):
    ...
    template_name = "business_rule/product_categories/product_category_list.html"
...
class ProductCategoryCreateView(ModalFormMixin, AdministrativeRequiredMixin, CreateView):
    ...
    template_name = "business_rule/product_categories/product_category_form.html"
...
class ProductCategoryDeleteView(AdministrativeRequiredMixin, DeleteView):
    model = ProductCategory
    template_name = "business_rule/lv/category_confirm_delete.html"
    success_url = reverse_lazy("system:product-category-list")
...
class ProductCategoryDetailView(AdministrativeRequiredMixin, DetailView):
    model = ProductCategory
    template_name = "business_rule/product_categories/product_category_detail.html"
