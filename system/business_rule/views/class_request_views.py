...
from system.business_rule.forms.class_request_forms import (
...
from system.business_rule.services.payroll_activation import (
...
class ExistingTeacherClassCatalogRequestCreateView(PortalRoleRequiredMixin, FormView):
    ...
    template_name = "business_rule/class_requests/request_form.html"
    success_url = reverse_lazy("system:home")
    required_capabilities = (Capability.SUPPORT_CLASSES,)

    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs["requester"] = portal_person(self.request)
        return kwargs
    ...
    def _save(self, form, extra_schedule_formset):
        ...
        try:
            create_existing_teacher_class_request(
                requester=portal_person(self.request),
...
class ClassCatalogRequestQueueView(PortalRoleRequiredMixin, ListView):
    ...
    template_name = "business_rule/class_requests/request_list.html"
...
class ClassCatalogRequestDetailView(PortalRoleRequiredMixin, DetailView):
    model = ClassCatalogRequest
    template_name = "business_rule/class_requests/request_detail.html"
    ...
    def _activate_payroll(self, request):
        ...
        try:
            activate_payroll_from_class_request(
                self.object,
                activated_by=portal_person(request),
