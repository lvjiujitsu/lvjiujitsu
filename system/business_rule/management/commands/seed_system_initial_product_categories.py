...
from system.business_rule.models import ProductCategory
...
class Command(BaseCommand):
    help = f"Cria as categorias de produto a partir de static/business_rule/initial_data/{DATA_FILENAME}."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\nCategorias de produto: {created_count} criada(s), {updated_count} atualizada(s)."
            )
        )
    ...
    def _apply_updates(self, category: ProductCategory, entry: dict) -> None:
