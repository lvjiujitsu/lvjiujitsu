...
from system.business_rule.models import Person, PersonType, PortalAccount
from system.business_rule.models.graduation import BeltRank, Graduation


class Command(BaseCommand):
    help = "Cria professores iniciais a partir de static/business_rule/initial_data/seed_system_initial_teachers.json."
    ...
    def handle(self, *args, **options):
        ...
        if not data:
            self.stdout.write(self.style.WARNING("Nenhum professor encontrado no arquivo seed_system_initial_teachers.json."))
            return
    ...
    def _load_json(self) -> list:
        path = Path(settings.BASE_DIR) / "static" / "business_rule" / "initial_data" / "seed_system_initial_teachers.json"
        if not path.exists():
