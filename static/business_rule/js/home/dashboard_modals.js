(function () {
  ...
  function bindClientProfileModal() {
    ...
    var ownerRoleBadgesSnapshot = null;
    if (roleBadgesEl) {
      ownerRoleBadgesSnapshot = document.createDocumentFragment();
      Array.prototype.forEach.call(roleBadgesEl.childNodes, function (node) {
        ownerRoleBadgesSnapshot.appendChild(node.cloneNode(true));
      });
    }

    function restoreRoleBadges(isOwner) {
      if (!roleBadgesEl) return;
      while (roleBadgesEl.firstChild) roleBadgesEl.removeChild(roleBadgesEl.firstChild);
      if (isOwner && ownerRoleBadgesSnapshot) {
        Array.prototype.forEach.call(ownerRoleBadgesSnapshot.childNodes, function (node) {
          roleBadgesEl.appendChild(node.cloneNode(true));
        });
        return;
      }
      var badge = document.createElement('span');
      badge.className = 'role-badge';
      badge.textContent = 'Dependente';
      roleBadgesEl.appendChild(badge);
    }
    ...
    function updateHeader(panel, person) {
      ...
      if (roleBadgesEl) {
        ...
        restoreRoleBadges(role === 'Titular');
      }
    }
