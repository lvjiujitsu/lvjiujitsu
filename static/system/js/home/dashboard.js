(function () {
  ...
  function showCheckinError(button, message) {
    ...
    error.textContent = message || 'NÃ£o foi possÃ­vel registrar o check-in.';
  ...
  function createPendingCheckinGroup(kind, id, personId) {
    ...
    group.appendChild(createStatusPill('Aguardando aprovaÃ§Ã£o', 'warning'));
  ...
  function handleStudentCheckin(button, config, csrfToken) {
    ...
    postJson(url, payload, csrfToken)
      ...
      .catch(function () {
        button.disabled = false;
        button.textContent = 'Check-in';
        showCheckinError(button, 'Falha de conexÃ£o ao registrar check-in.');
      });
  }
  ...
  function handleStudentCheckinCancel(button, config, csrfToken) {
    ...
    postJson(url, payload, csrfToken)
      ...
      .catch(function () {
        button.disabled = false;
        if (label) label.textContent = 'Desfazer';
        showCheckinError(button, 'Falha de conexÃ£o ao desfazer check-in.');
      });
  }
      ...
      .then(function (result) {
        ...
        if (classItem) {
          ...
          if (summary) {
            ...
            if (pendingCount > 0) {
              summary.textContent += ' Â· ' + pendingCount + ' pendente' + (pendingCount === 1 ? '' : 's');
            }
          }
  ...
  function bindGradDetailsToggle() {
    document.addEventListener('click', function (e) {
      ...
      if (icon) icon.textContent = next ? 'âˆ’' : '+';
    });
  }
  ...
  function bindSectionCollapse() {
    ...
    try {
      ...
      if (typeof console !== 'undefined' && console.warn) {
        console.warn('[LV dashboard:storage]', 'Falha ao ler preferÃªncias de seÃ§Ã£o', e);
      }
    }
    ...
    document.querySelectorAll('.section__toggle').forEach(function (btn) {
      ...
      btn.addEventListener('click', function () {
        ...
        try {
          ...
          if (typeof console !== 'undefined' && console.warn) {
            console.warn('[LV dashboard:storage]', 'Falha ao salvar preferÃªncias de seÃ§Ã£o', e);
          }
        }
      });
    });
