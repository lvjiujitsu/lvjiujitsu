...
The cleanup process matcher accepts an unrelated manage.py process using the shared port. Settings also fail to locate the shared environment documented by the reset contract.
...
Added read-only --check with complete preflight before mutations. Removed port-only ownership inference; foreign processes remain untouched. Reject remote environment files even with a local process override and resolve explicit shared environment files consistently. The documented shared settings loader and the environment/schema check commands are now implemented. HG and production reset commands now share confirmation, project-ref, host and execution guards; production additionally requires --confirm-ref. All destructive behavior was exercised with temporary fixtures or mocks.
