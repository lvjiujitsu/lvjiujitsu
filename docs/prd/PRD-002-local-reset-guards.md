# PRD-002: Keep local reset within its project and environment

## Context

The cleanup process matcher accepts an unrelated manage.py process using the shared port. LV settings also fail to locate the shared environment documented by the reset contract.
...
clear_migrations.py, system/tests/test_tooling_parity.py, system/tests/test_remote_reset_parity.py, system/management/commands/_supabase_public_schema_reset.py and HG/prod subclasses; LV settings loader and missing environment/schema check commands; Obsidian database operation notes.
...
clear_migrations.py, system/tests/test_tooling_parity.py, system/tests/test_remote_reset_parity.py, system/management/commands/_supabase_public_schema_reset.py and HG/prod subclasses; LV settings loader and missing environment/schema check commands; Obsidian database operation notes. Exact paths and before/after hashes are in paritydjango/validation/changes.json; initial user changes are distinguished from this audit.
...
Added read-only --check with complete preflight before mutations. Removed port-only ownership inference; foreign processes remain untouched. Reject remote environment files even with a local process override and resolve explicit shared environment files consistently. LV now implements its documented shared settings loader and the environment/schema check commands. HG and production reset commands now share confirmation, project-ref, host and execution guards; production additionally requires --confirm-ref. All destructive behavior was exercised with temporary fixtures or mocks.
