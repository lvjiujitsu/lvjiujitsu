...
Auditoria isolada executada em 2026-06-30. Fontes: `AGENTS.md`, `CLAUDE.md`, contratos em `docs/`, código em `system/`, inventário de templates, testes locais, comparação Obsidian × `OPERACAO-BANCO-SEEDS.md`, referência Visary (padrões) e snapshot legado em `Downloads/lvjiujitsu-fef9b13890c09fc822baf1e9063a319dd16e14a3`.
...
10. **C** — Recuperação de senha: 4 templates ausentes apesar de rotas ativas. Evidência: `auth_views.py:732-761`, `UI-SCREEN-CONTRACT.md` §9.1.
...
12. **A** — Rotas 100% em português (`pessoas/`, `turmas/`, `financeiro/`) divergem de PRD-075 e padrão Visary (`clients/`, `partners/`). Evidência: `system/urls.py`, `visary/system/urls.py`.
...
77. **B** — `UI-SCREEN-CONTRACT.md` §9.2 marca dashboards separados como pendentes; código usa home unificada — contrato desatualizado.
...
### L. Referência Visary — padrões a seguir (informativo)

91. **—** — Rotas em inglês e substantivos no plural (`clients/`, `partners/`).
92. **—** — CRUD curto em modal com `?modal=1` + `postMessage` (`visary` views modais).
...
98. **—** — Qualquer regra de negócio de consultoria/domínio Visary.
