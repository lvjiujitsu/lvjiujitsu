# PRD-054: Alinhamento Arquitetural LV × Visary

## Resumo do que será implementado

Corrigir os bugs críticos nos arquivos de ambiente do LV que impedem o servidor de subir com `.env.hg` e inviabilizam o reset seguro do Supabase. Em seguida, portar para o Visary os componentes arquiteturais superiores identificados no LV (base class de reset, settings robustos, `.env.example` completo). O resultado final é dois projetos-irmãos com governança de ambiente idêntica e sem divergências que quebram deploy ou reset.

## Tipo de demanda

Correção pontual (bugs bloqueantes) + alteração arquitetural (alinhamento entre projetos-irmãos) + revisão de governança.
...
### Componentes superiores no LV ausentes no Visary

| Componente | LV | Visary | Impacto da ausência no Visary |
|---|---|---|---|
...
### Componentes superiores no Visary ausentes no LV

| Componente | Visary | LV | Impacto da ausência no LV |
|---|---|---|---|
| `DATABASE_URL` com `sslmode=require` | `?sslmode=require` na connection string | ausente | Conexão Supabase pode falhar em alguns contextos de rede |
| `DATE_INPUT_FORMATS` | Configurado (`%Y-%m-%d`, `%d/%m/%Y`, etc.) | ausente | Parsing de datas de formulário menos tolerante |
| `DATE_FORMAT` / `DATETIME_FORMAT` | `d/m/Y` e `d/m/Y H:i` | ausentes | Formatação de datas inconsistente no template |
| `STORAGES` condicional | WhiteNoise só em produção; `StaticFilesStorage` em DEBUG | WhiteNoise sempre | `collectstatic` obrigatório mesmo em desenvolvimento |
...
3. Atualizar o Visary: base class de reset, settings robustos, `.env.example` completo
...
**Visary (`C:\Users\whsf\Documents\GitHub\visary`):**
- `visary/settings.py`
- `.env.hg`
- `.env.prod`
- `.env.example`
...
### Contexto
LV e Visary são projetos-irmãos com mesma stack (Python 3.12 + Django 5.2 LTS) e mesma infraestrutura (Render + Supabase). As divergências são fruto de evolução independente após o ajuste de deploy. O objetivo é convergir a governança de ambiente para um padrão único e verificável.
...
- Mudanças no `settings.py` do Visary não devem quebrar os testes existentes
...
- [ ] `clear_migration_supabase_hg.py` e `clear_migration_supabase_prod.py` do Visary reescritos como subclasses
- [ ] `visary/settings.py` contém: validações de boot (`ImproperlyConfigured`), `LocMemCache`, `SESSION_ENGINE=cached_db`, `cached.Loader` em produção, logging filtrado, `WHITENOISE_MAX_AGE`
- [ ] `.env.example` do Visary atualizado com todos os campos necessários
- [ ] `manage.py test --verbosity 2` do Visary passa sem falhas após mudanças de settings
...
- `manage.py check` (Visary com `.env.hg`) sem erros
- `manage.py test --verbosity 2` (Visary) — 0 falhas, 0 erros
...
4. Atualizar `visary/settings.py`:
...
5. Atualizar `.env.example` do Visary com todos os campos
...
| `visary/settings.py` | Edição: validações, cache, sessão, template loader, logging, WhiteNoise |
...
O `dj_database_url.parse()` do LV lê `sslmode` do query string e passa para `OPTIONS`. Não há conflito. O `_parse_db_url` do Visary também lê `sslmode` explicitamente. Em ambos os casos, adicionar `?sslmode=require` é seguro.

Se o `DATABASE_URL` já tiver `?` (outros params), deve-se usar `&sslmode=require`. Verificar antes de editar.

### `cached.Loader` no Visary
O Visary usa `APP_DIRS=True` atualmente. Mudar para `cached.Loader` em produção exige remover `APP_DIRS` e listar os loaders explicitamente. Os testes do Visary rodam em DEBUG — não são afetados.
...
### Mudanças no Visary `settings.py` — impacto em testes
Os testes do Visary rodam com `DEBUG=True` (SQLite). As mudanças de settings são condicionadas a `if not DEBUG` — sem impacto nos testes.
...
### Bloco 3 — Visary: base class de reset
- [ ] 3.1. Criar `system/management/commands/_supabase_public_schema_reset.py` (cópia do LV)
- [ ] 3.2. Reescrever `clear_migration_supabase_hg.py` como subclasse
- [ ] 3.3. Reescrever `clear_migration_supabase_prod.py` como subclasse
- [ ] 3.4. Validar: `manage.py check` (Visary local) — 0 erros

### Bloco 4 — Visary: settings robusto
- [ ] 4.1. Adicionar validações de boot: `DJANGO_ENVIRONMENT`, `DEBUG=False` para hg/prod, `SECRET_KEY`, `DATABASE_URL`
- [ ] 4.2. Adicionar `LocMemCache`
- [ ] 4.3. Adicionar `SESSION_ENGINE=cached_db`
- [ ] 4.4. Adicionar `cached.Loader` em produção
- [ ] 4.5. Adicionar logging filtrado em produção
- [ ] 4.6. Adicionar `WHITENOISE_MAX_AGE`
- [ ] 4.7. Tornar `STORAGES` condicional
- [ ] 4.8. Validar: `manage.py test --verbosity 2` (Visary) — 0 falhas, 0 erros
- [ ] 4.9. Validar: `manage.py check` (Visary com `.env.hg`) — 0 erros

### Bloco 5 — Visary: `.env.example`
- [ ] 5.1. Atualizar `.env.example` do Visary com todos os campos necessários (baseado no LV)
...
- [ ] 6.3. Atualizar `CLAUDE.md` do Visary se existir, ou registrar achado
...
### Visary
```powershell
# Em C:\Users\whsf\Documents\GitHub\visary
$env:DJANGO_ENV_FILE=".env.hg"
.\.venv\Scripts\python.exe manage.py check
# Esperado: System check identified no issues.
...
- [x] `manage.py check` (local) → `System check identified no issues (0 silenced).`
- [x] `manage.py check` com `.env.hg` → `System check identified no issues (0 silenced).`
- [x] `manage.py test --verbosity 2` → 86 testes, 0 falhas, 0 erros
...
### Visary
- `system/management/commands/_supabase_public_schema_reset.py`: criado com `SupabasePublicSchemaResetCommand` base class (cópia fiel do LV — `transaction.atomic`, dropa tabelas, views, materialized views, sequences, foreign tables)
- `system/management/commands/clear_migration_supabase_hg.py`: reescrito como subclasse (3 linhas)
- `system/management/commands/clear_migration_supabase_prod.py`: reescrito como subclasse (3 linhas)
- `visary/settings.py`: reescrito com validações de boot (`ImproperlyConfigured` para `DJANGO_ENVIRONMENT`, `DJANGO_DEBUG` em hg/prod, `SECRET_KEY`, `DATABASE_URL`), `LocMemCache`, `SESSION_ENGINE=cached_db`, `cached.Loader` em produção, logging filtrado em produção, `WHITENOISE_MAX_AGE`, `STORAGES` condicional, `SECURE_PROXY_SSL_HEADER`, CSRF/Session cookie settings, `DB_CONN_MAX_AGE`, `CONN_HEALTH_CHECKS`; preservados todos os componentes específicos do Visary (`_parse_db_url`, `LEGACY_DB_*`, `SYSTEM_SEED_*`, `_env_value_strip_outer_quotes`, `DATE_INPUT_FORMATS`, `DATE_FORMAT`, `DATETIME_FORMAT`)
- `.env.example`: atualizado com todos os campos necessários
